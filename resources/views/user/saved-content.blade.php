@extends('layouts.app')

@section('content')
    {{-- @if(count($saved_contents) > 0)
        @foreach($saved_contents as $saved_content)
            <div class="card-deck" style="margin-top:20px;margin-bottom:5px;width:30%;">
                <a href="/contents/{{$saved_content->id}}">
                <div class="card" style="margin:0;width:95%;">
                    @if (empty($saved_content->content_img))
                        <img src="https://www.litmos.com/wp-content/uploads/2016/06/blog-eLearning-templates.png" alt="post_image" class="card-img">
                    @else
                        <img src="{{ url('/data_file/images/'.$saved_content->content_img) }}" alt="post_image" class="card-img">
                    @endif
                    <div class="card-body">
                        <p class="card-title">{{$saved_content->title}}</p>
                        <hr>
                        <div class="text-card">
                            @if (empty($saved_content->description))
                                <p class="card-text"><small>Tidak ada deskripsi</small></p>
                            @else
                                <p class="card-text"><small>{{$saved_content->description}}</small></p>
                            @endif
                            
                        </div><br>
                        <div class="rating-card">
                            <img id="rating" src="data:image/svg+xml;utf8;base64," />
                            @if (empty($saved_content->rating))
                                <p id="ratVal" style="font-size:15px"><small style="font-size:8px">belum ada rating</small> </p>
                            @else
                                <p id="ratVal" style="font-size:15px"> {{$saved_content->rating}} <small style="font-size:8px">/10</small> </p>
                            @endif
                        </div>
                    </div>
                    <div class="card-footer">
                        <small class="text-muted" style="position:relative">Dibuat pada: {{date('d-m-Y', strtotime($saved_content->created_at))}}</small>
                    </div>
                </div>
                </a>
            </div>
        @endforeach     
        {{$saved_contents->links()}}   
    @else
        <div class="col-md-12" style="text-align:center;">
            <br><br><br><br><br>
            <h3>Tidak ada konten yang anda simpan</h3>
            <a href="/contents">Lihat konten pembelajaran di sini</a>
        </div>
    @endif --}}
    {{-- {{$saved_contents}} --}}
    <div class="container-fluid">
        @include('inc.sidebar-profile')
        <div class="content">
            @if(count($saved_contents) > 0)
                @foreach($saved_contents as $saved_content)
                <div class="card mb-3">
                        <div class="row no-gutters">
                            <div class="col-md-4">
                            @if (empty($saved_content->content_img))
                                <img src="https://www.litmos.com/wp-content/uploads/2016/06/blog-eLearning-templates.png" alt="post_image" class="card-img-top" height="250">
                            @else
                                <img src="{{ url('/data_file/images/'.$saved_content->content_img) }}" alt="post_image" class="card-img-top" height="250">
                            @endif
                            </div>
                            <div class="col-md-8">
                            <div class="card-body">
                                <h3 class="card-title">{{ $saved_content->title }}</h3>
                                @if (empty($saved_content->description))
                                    <p class="card-text"><small>Tidak ada deskripsi</small></p>
                                @else
                                    <p class="card-text"><small>{{$saved_content->description}}</small></p>
                                @endif
                                @if (empty($saved_content->rating))
                                    <p class="mb-0">
                                        <img src="data:image/svg+xml;utf8;base64," height="25"/>
                                        <small>belum ada rating</small>
                                    </p>
                                @else
                                    <p class="mb-0">
                                        <img src="data:image/svg+xml;utf8;base64," height="25"/>
                                        {{ $saved_content->rating }}
                                        <small>/10</small>
                                    </p>
                                @endif
                            </div>
                            <div class="card-footer" style="background:none; border:none;">
                                <a href="/contents/{{$saved_content->id}}" class="btn btn-success stretched-link col-md-12">Pelajari</a>
                            </div>
                            <div class="card-footer">
                                <small class="text-muted">Dibuat pada: {{date('d-m-Y', strtotime($saved_content->created_at))}}</small>
                            </div>
                            </div>
                        </div>
                        </div>
                @endforeach
                {{ $saved_contents->links() }}        
            @else
                <div class="col-md-12" style="text-align:center;">
                    <br><br><br><br><br>
                    <h3>Tidak ada konten yang anda buat</h3>
                </div>
            @endif
        </div>
    </div>
@endsection